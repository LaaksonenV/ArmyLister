...
void ListCreator::CreateArmy(QWidget *parent)
{
    ...
    creator.info_->setText(
                ...
                "created previously (or later, though "
                ...
                "If entrys text is left empty or ends with ':', its text will "
                "update automatically based on selected items under it, "
                "empty text being replaced, otherwise appended afetr ':'"
                ...
                "controls.\n"
                "You can add parameter elements to list with '!(X)' "
                "or '!('X'='default')', and when using list add '!(AA)' to the "
                "control elements. "
                "(Don't include any special characters in 'AA' or 'X')"
                "Then every instance of 'X' in lists items control elements will"
                "be X (or 'default' if no '(AA)' is given, or 'X' if neither). "
                "For now only one parameter may be given. More needed?\n"
                "Creating a list with existing name will append items to "
                "escisting list, new parameters will override old.\n"
