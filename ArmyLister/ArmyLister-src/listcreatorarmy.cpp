...
ListCreatorArmy::ListCreatorArmy(QWidget *parent)
    : QDialog(parent)
    , v_currentIndex(-1)
...
void ListCreatorArmy::writeFile()
{
    QFile f(c_fileName);
...
QString ListCreatorArmy::getDetachmentList()
{
    QFile f(c_fileName);
    if (!f.open(QFile::Text | QFile::ReadOnly))
        return QString();
...
void ListCreatorArmy::on_AddItem()
{
    addItem();
}

void ListCreatorArmy::removeItem()
{
    if (v_currentIndex < 0)
        return;
...
void ListCreatorArmy::specialSelected(int index)
{
    Q_UNUSED(index)
}

void ListCreatorArmy::itemSelected(int index)
{
    saveCurrent();
...
void ListCreatorArmy::saveCurrent()
{
    if (v_currentIndex < 0)
        return;
